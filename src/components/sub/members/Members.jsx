...
/* 
	react-hook-form을 쓰지않고 직접 기능을 만들었냐?
	-- 라이브러리는 언제든지 연결할 수 있는건데, 아직 배우는 입장이기 때문에 부족하나마 어떻게 인증로직이 처리되는지 직접 만들어보고싶었다.

	그래서 checkbox, radio, selector, textarea 같이 필수 입력사항이 아닌 요소도 직접 인증구현을 해봤다.
	인증처리 하면서 제일 힘들었던 부분을 비밀번호, 이메일 인증 구현이 힘들었다.

	구글링을 해보니 정규표현식으로 예시코드가 많이 있었지만 아직 정규표현식을 제대로 공부한것이 아니라 모르는 상태에서 붙여넣기 식으로 구현하기는 싫어서
	내가 알고 있는 문자열 관련 메서드를 최대한 활용해서 구현해 봤다.
*/
