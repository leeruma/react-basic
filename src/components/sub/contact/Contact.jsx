...
export default function Contact() {
	...
	// kakao api를 cdn 방식으로 불러오고 있기 떄문에 리액트 컴포넌트가 실ㄹ행되면 window객체에서 직접 비구조화 할당으로 kakao객체를 뽑아옴
	const { kakao } = window;
	//첫번째 지도를 출력하기 위한 객체정보

	//지도정보데이터를 객체형식으로 구조화한 다음에 데이터 기반으로 자동 지도 화면이 생성되도록 만들었다.
	// 데이터정보가 많아질때를 대비해서 유지보수에 최적화 되도록 코드 개선
	// 해당 정보값은 자주 바뀌는 값이 아니기 때문에 굳이 state에 담아서 불필요한 재랜더링을 막기위해 useRef에 담아놨다
	const info = useRef([
		{
			title: '삼성역 코엑스',
	...
	useEffect(() => {
		...
		// 지도 생성시 마커 고정적으로 적용되기 떄문에 브라우저 리사이즈시 마커가 가운데 위치하지 않는 문제
		...
		// 해결방법: Contact컴포넌트가 언마운트시 강제로 윈도우객체에서 setCenter핸들러를 제거
	...
	const sendEmail = (e) => {
		...
		// sendForm메서드는 각 키값을 문자열로만 인수로 전달되도록 type지정되어있기 때문에
		// 변수를 `${}` 로 감싸서 문자형식으로 전달
		emailjs
			.sendForm(
...
/*
	...
	- cdn으로 받아온 kakao생성자 함수를 컴포넌트 안쪽에서 불러와지지 않는 문제가 있어서 window객체로 부터 직접 비구조화할당으로 뽑아와서 활용했다.

	kakao생성자를 통해서 만들어진 지도 인스턴스값을 state에 담아서 각각의 이벤트에 연결했다.
	작업을 하다보니 아무래도 리액트로 작업하는 프로젝트는 지점이 많은 대형프로젝트일것 같아서 지점 버튼 클릭시 다른 지도를 출력하도록 구현했는데 너무 코드가 지저분해져서 지도 정보값을 배열형태로 묶어서 추후 배열 데이터가 변경이 되면 데이터 기반으로 자동으로  새로운 지도 인스턴스 생성과 이벤트 연결까지 한번에 처리되도록 자동화 시키는데 중점을 뒀다.

	이슈사항2
	- 브라우저 리사이즈시 마커가 가운데 가지 않아서 브라우저 리사이즈 이벤트 발생할 때마다 마커가 가운데 위치하는 함수를 재호출했다.

	- window객체에 이벤트 연결하다보니 리사이즈 이벤트가 발생할 필요가 없는 다른 컴포넌트에서도 핸들러함수가 호출되는 문제점이 있어서 컴포넌트 언마운트시 윈도우객체에 이벤트 핸들러 제거했다.
*/
