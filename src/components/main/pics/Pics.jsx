...
function Pics() {
	...
	const handleScroll = () => {
		...
		// 해당 스크롤값이 Pics영역에 도달했을때 0으로 보정해놓은 값
	...
	return (
		// 색션의 높이값은 자식 요소의 4개 div요소 넓이값을 합친 총합의 크기만큼 높이값을 확보해야됨 (그 크기 만큼 Pics영역안에서 스크롤 이벤트를 발생시켜야 하기 떄문)

		// 스크롤 해당 Pics영역에 도달하게 되면 Pics안쪽의 article 프레임을 position: absolute에서 position: fixed로 속성을 변경
		// article이 브라우저단에 세로 0px 위치로 고정이 되면서 마치 가로로 스크롤이 움직이는 것 같은 효과 구현
		// Pics영역을 스크롤이 벗어나면 다시 absolute속성으로 변경해ㅐ서 다시 위쪽으로 움직이도록 배치
